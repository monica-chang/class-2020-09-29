...
**Prompt:** We could copy/paste this code 5 times, adjust the files names, and then read in each file. But, as you know from Chapter 4, that is a bad idea. It also scales very poorly. Create a function called `read_enrollment` which takes one argument, `file`. Use that function to read in the data from fall 2019 and assign it to an object called `d_2019_take_2`. Do you get the same answer as you did in Scene 1?

```{r sc2}

read_enrollment <- function(file, n = 3) {
  read_excel(path = file, skip = n) %>% 
  clean_names() %>%
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
         -withdraw, -total, -instructor_full_name, -course_section_code) %>%
...
d_2018 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", n = 2)

d_2017 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.20.17.xlsx")

d_2016 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2016.xlsx", n = 0)
...
bind_rows(list(d_2019_take_2, d_2018, d_2017, d_2016, d_2015), .id = "year") %>%
...
bind_rows(list(d_2019_take_2, d_2018, d_2017, d_2016, d_2015), .id = "year") %>%
  mutate(year = as.numeric(year) + 2014) %>%
  filter(department == "Economics" | department == "Government" | 
         department == "Computer Science") %>%
  group_by(department, year) %>%
  summarize(students_in_dept = sum(u_grad), .groups = "drop") %>%
  ggplot(aes(x = year, y = students_in_dept, group = department, color = department)) + 
    geom_line() +
    theme_bw() +
    labs(
      title = "Increase/Decrease in Econ, CS, and Gov Enrollment from 2015-2019",
      x = "Year",
      y = "Students Enrolled in the Department")
